...
```{.python}
class Article:
    def __init__(self, reference, prix_ht):
    ...
    def modifier_prix_ht(self, nouveau_prix):
        '''Setter'''
        if nouveau_prix > 0:
            self.__prix_ht = nouveau_prix
...
if __name__ == "__main__":
    ...
    # print(a.__prix_ht)
    # 'Article' object has no attribute '__prix_ht'

    a.modifier_prix_ht(10)
    print(a.obtenir_prix_ht())
    # 10
```
...
Dans l'exemple ci-dessus, l'attribut *\_\_prix\_ht* deviendra *\_Article\_\_prix\_ht*.

```{.python}
a = Article("A12B5", 5)
print(a._Article__prix_ht)
# 5
```

Si vous tentez de modifier l'attribut *\_\_prix\_ht*, cela va en fait créer un nouvel attribut
